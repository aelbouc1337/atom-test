...
const Cart: React.FC<CartProps> = ({ showCart, setShowCart }) => {
  ...
  return (
    <motion.div
      ...
      className="absolute top-28 right-0 w-[90%] lg:w-[40%] p-8 bg-white border rounded-3xl shadow-lg z-50"
    >
        ...
        {cart.map((product) => (
          <li className="px-3 py-3 bg-white flex shadow-md border rounded-3xl justify-between items-center">
            <div className="flex flex-row gap-2 items-center font-inter">
              <Image src={product.image} width={70} height={70} alt="picture" />
      ...
      <div className="w-full flex mt-9 items-center justify-between">
        ...
        <button
          ...
          className={`px-5 py-4 flex items-center justify-center gap-1 bg-black text-white rounded-xl font-inter tracking-widest ${
